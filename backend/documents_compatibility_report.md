...
| `doc.status`            | `status`           | ✅ متطابق              | القيم متوافقة: `valid`, `suspend`                                              |
...
| `showFilter` → `"valid"`   | صالح            | `show` → ❌ **غير موجود**                         | ❌ **غير متوافق** — الباك يدعم: `all`, `suspend`, `expired`, `not_yet_expired`, `expired_on` — لا يوجد `valid` |
| `showFilter` → `"suspend"` | موقوف           | `show` → `suspend`                                | ✅ متطابق                                                                                                      |
...
| ----------------- | ------------------------------------------------- | ------------ | -------------------------------------------------------------------- |
| `Suspend`         | `action: "suspend"`                               | ❌ **مختلف** | الفرنت يسميها `Suspend` (حرف كبير) والباك يتوقع `suspend` (حرف صغير) |
...
| --------------------------------------- | ------------------------------------------------ | ------------ |
| `{ ids: [1, 2, 3], status: 'suspend' }` | `{ document_ids: [1, 2, 3], action: 'suspend' }` | ❌ **مختلف** |
...
> 3. قيم الإجراءات: `Suspend/Activate/Delete` ← يجب أن تصبح `suspend/active/deleted`
...
3. إضافة فلتر `valid` في `IndexDocumentRequest` ← `show => 'in:all,valid,suspend,...'`
...
6. تغيير أسماء الـ Bulk Actions: `Suspend→suspend`, `Activate→active`, `Delete→deleted`
