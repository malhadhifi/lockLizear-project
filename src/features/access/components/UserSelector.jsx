...
const StatusBadge = ({ status }) => {
  const statusConfig = {
    ...
    suspended: { class: 'badge-suspended', icon: 'pause-circle-fill', text: 'موقوف' },
  ...
  return (
    <div className="h-100 d-flex flex-column">
      ...
      <div className="flex-grow-1 overflow-auto p-2">
        {filteredUsers.length === 0 ? (
          <div className="text-center p-5">
            <i className="bi bi-people" style={{ fontSize: '40px', color: '#d1d5db' }} />
          ...
          filteredUsers.map((user) => (
            <div
              ...
              style={{
                border: selectedUserId === user.id ? '2px solid var(--primary)' : '1px solid #e9ecef',
                background: selectedUserId === user.id ? '#eef2ff' : '#fff',
                cursor: 'pointer',
                transition: 'all 0.2s',
              }}
              onClick={() => onUserSelect(user)}
              onMouseOver={e => { if (selectedUserId !== user.id) e.currentTarget.style.background = '#f8f9fa' }}
              onMouseOut={e => { if (selectedUserId !== user.id) e.currentTarget.style.background = '#fff' }}
            >
              {/* اسم المستخدم + حالة */}
              ...
              {/* البريد الإلكتروني */}
              <div className="text-muted mb-2 text-truncate" style={{ fontSize: '12px' }}>
                {user.email}
              </div>
      ...
      <div className="p-3 border-top">
        <div className="text-muted d-flex justify-content-between align-items-center" style={{ fontSize: '12px' }}>
