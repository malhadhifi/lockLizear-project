...
const PropertiesTab = ({ properties, onUpdate, onSave }) => {
  ...
  // ===== Countries List =====
  const countries = [
    'اليمن', 'السعودية', 'الإمارات', 'قطر', 'الكويت', 'عمان',
    'مصر', 'الأردن', 'لبنان', 'العراق', 'السودان'
  ]
  ...
  // ===== إضافة دولة =====
  const addCountry = () => {
    if (countryInput.trim() && !properties.countryRestrictions.includes(countryInput.trim())) {
      const newCountries = [...properties.countryRestrictions, countryInput.trim()]
  ...
  const removeCountry = (country) => {
    const newCountries = properties.countryRestrictions.filter(c => c !== country)
    onUpdate({ countryRestrictions: newCountries })
  }
  ...
  return (
    <div>
      ...
      <div className="drm-card mb-4">
        ...
        <div className="card-body">
          <div className="form-check form-switch mb-3">
            ...
            />
            <label className="form-check-label fw-semibold" htmlFor="offlineAccess">
            ...
            <div className="row g-3 mt-1">
              <div className="col-md-6">
                ...
                <input
                  ...
                  onChange={(e) => onUpdate({ offlineCheckinDays: parseInt(e.target.value) })}
                />
              </div>
            </div>
          )}
        ...
        <div className="card-body">
          <div className="d-flex gap-2 mb-3">
            ...
            <button
              ...
              onClick={addIp}
            >
              <i className="bi bi-plus-lg me-1" />إضافة
            </button>
          </div>
          ...
          <div className="d-flex flex-wrap gap-2">
            {properties.ipRestrictions.map((ip, index) => (
              <span
                ...
                style={{ background: '#e8f0ff', color: 'var(--primary)', fontSize: '13px' }}
              >
                {ip}
                <i
            ...
            {properties.ipRestrictions.length === 0 && (
              <span className="text-muted" style={{ fontSize: '13px' }}>لا توجد قيود IP حالياً</span>
            )}
          </div>
        </div>
      </div>
          ...
          <div className="d-flex flex-wrap gap-2">
            {properties.countryRestrictions.map((country, index) => (
              <span
                key={index}
                className="badge rounded-pill px-3 py-2 d-flex align-items-center gap-2"
                style={{ background: '#fff3e8', color: '#d4600a', fontSize: '13px' }}
              >
                {country}
                <i
                  className="bi bi-x-lg"
            ...
            {properties.countryRestrictions.length === 0 && (
              <span className="text-muted" style={{ fontSize: '13px' }}>لا توجد قيود دول حالياً</span>
            )}
          </div>
        </div>
      </div>
      ...
      <div className="d-flex gap-2">
        <button 
          className="btn btn-primary-drm flex-grow-1"
          onClick={onSave}
        >
