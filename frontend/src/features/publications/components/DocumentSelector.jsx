...
const DocumentSelector = ({ isOpen, onClose, existingDocIds = [], onDocumentsAdded }) => {
  ...
  return (
    <div style={{
      ...
      <div style={{ width: '100%', maxWidth: 550, maxHeight: '80vh', display: 'flex', flexDirection: 'column', background: '#fff', border: '1px solid #999' }}
        ...
        {/* بحث */}
        <div style={{ padding: '10px 16px', background: '#f0f0f0', borderBottom: '1px solid #ccc' }}>
          <div className="d-flex align-items-center gap-1">
            <label style={{ fontWeight: 600, fontSize: 13 }}>فلتر:</label>
            <input type="text" value={filter} onChange={e => setFilter(e.target.value)}
              placeholder="البحث في المستندات..."
              style={{ border: '1px solid #999', borderRadius: 3, padding: '4px 8px', flex: 1, fontSize: 13 }} />
          </div>
        </div>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
            ...
            <tbody>
              {isLoading ? (
                ...
                <tr key={d.id} style={{
                  background: selected.includes(d.id) ? '#e3f2fd' : (idx % 2 === 0 ? '#fff' : '#f5f5f5'),
        ...
        <div style={{
          ...
          <div className="d-flex gap-2">
            <button onClick={onClose}
              style={{ background: '#f0f0f0', color: '#333', border: '1px solid #999', borderRadius: 3, padding: '6px 16px', cursor: 'pointer' }}>
              إلغاء
            </button>
