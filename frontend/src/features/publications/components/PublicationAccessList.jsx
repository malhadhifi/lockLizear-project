...
import {
  usePublicationSubscribers,
  useRevokeSubscriberAccess,
...
const PublicationAccessList = ({ publicationId }) => {
  ...
  const customers = Array.isArray(subsRes?.data)
    ? subsRes.data
  ...
  const { data: allCustomersRes } = useQuery({
    ...
    queryFn: () =>
      api.get("/customer-management/customer-licenses", {
        params: { limit: 50, search: filter },
      }),
  });
  const allCustomers =
    allCustomersRes?.data?.items || allCustomersRes?.items || [];
  ...
  const displayList = useMemo(() => {
    ...
    if (showFilter === "all") {
      ...
      list = customers
        .filter((c) => !c.valid_from && !c.valid_until)
        .map((c) => ({ ...c, hasAccess: true, accessType: "unlimited" }));
    } else if (showFilter === "limited") {
      list = customers
        .filter((c) => c.valid_from || c.valid_until)
        .map((c) => ({ ...c, hasAccess: true, accessType: "limited" }));
    }
    return list;
  ...
  const invertSelection = () =>
    setSelected(
      filtered.map((c) => c.id).filter((id) => !selected.includes(id)),
    );
  ...
  const handleBulkAction = () => {
    ...
    if (bulkAction === "revoke") {
      revokeMutation.mutate(
        { id: publicationId, customer_ids: selected },
        {
          onSuccess: () => {
  ...
  const statusLabel = (status) => {
    const map = {
      registered: {
        color: "#2e7d32",
        bg: "#e8f5e9",
        text: "مسجل (Registered)",
      },
      suspend: { color: "#e65100", bg: "#fff3e0", text: "موقوف (suspend)" },
      expired: { color: "#c62828", bg: "#ffebee", text: "منتهي (Expired)" },
      not_registered: {
        color: "#1565c0",
        bg: "#e3f2fd",
        text: "غير مسجل (Not Registered)",
      },
    };
  ...
  return (
    <div>
      ...
      {isLoading && (
        <div style={{ textAlign: "center", padding: 20, color: TEAL }}>
          جارٍ جلب عملاء المنشور... ⏳
        </div>
      )}
      ...
      >
        ...
        >
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            ...
            <select
              value={showAtLeast}
              onChange={(e) => setShowAtLeast(Number(e.target.value))}
              style={{
                border: "1px solid #ccc",
                borderRadius: 3,
                padding: "4px 8px",
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            ...
            >
              ...
              <option value="unlimited">
                وصول دائم (With Unlimited Access)
              </option>
        ...
        >
          ...
          >
            <option value=""></option>
            <option value="grant_unlimited">
              Grant Unlimited Access (وصول دائم)
            </option>
            <option value="grant_limited">
              Grant Limited Access (وصول محدود)
            </option>
            ...
            >
              ...
              <div style={{ flex: 1, padding: "10px 16px", fontSize: 13 }}>
                ...
                <table style={{ fontSize: 13, lineHeight: 1.7 }}>
                  <tbody>
                    <tr>
                      <td style={flStyle}>الشركة (Company):</td>
                      <td>{c.company || "—"}</td>
                    </tr>
                    <tr>
                      <td style={flStyle}>البريد (Email):</td>
                      <td>{c.email || "—"}</td>
                    </tr>
                    <tr>
                      <td style={flStyle}>المعرف (ID):</td>
                      <td>{c.id}</td>
                    </tr>
                    <tr>
                      <td style={flStyle}>الحالة (Status):</td>
                      <td>{statusLabel(c.status)}</td>
                    </tr>
                    <tr>
                      <td style={flStyle}>تاريخ البدء (Valid From):</td>
                      <td>{c.valid_from || c.registered || "—"}</td>
                    </tr>
                    <tr>
                      <td style={flStyle}>تاريخ الانتهاء (Valid Until):</td>
                      <td>{c.valid_until || c.expires || "—"}</td>
                    </tr>
                    {c.hasAccess !== undefined && (
                      <tr>
                        <td style={flStyle}>نوع الوصول (Access):</td>
                        <td
                          style={{
                            color: c.hasAccess ? "#4caf50" : "#999",
                            fontWeight: 600,
                          }}
                        >
                          {c.hasAccess
