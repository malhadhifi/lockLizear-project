...
export default function UserListHeaderAndFilters({
  filter, setFilter,                 // حالة البحث النصي ودالة تحديثها
  sortBy, setSortBy,                 // حالة الترتيب (الاسم، الشركة، المعرف) ودالة تحديثها
  showAtLeast, setShowAtLeast,       // حالة عدد السجلات المعروضة بالصفحة
  showFilter, setShowFilter,         // حالة فلتر ظهور الحسابات (مسجل، موقوف، الخ)
  setCurrentPage,                    // دالة إرجاع المستخدم للصفحة رقم 1 عند تغيير أي فلتر
  checkAll, uncheckAll, invertSelection, // دوال للتحكم في تحديد مربعات الاختيار في الجدول
  bulkAction, setBulkAction,         // حالة الإجراء الجماعي المختار ودالة تحديثه
  setSelectedResource,               // دالة لحفظ المورد (منشور/مستند) المختار للإجراء
  setIsPubModalOpen, setIsDocModalOpen, // دوال تفتح قوائم اختيار المنشورات أو المستندات
  selectedResource, selected,        // المورد المحدد حالياً وقائمة العملاء المحددين في الجدول
  ...
  return (
    <>
      ...
      {/* ========================================================= */}
      <div style={{
        background: TEAL, color: '#fff', padding: '10px 16px', // الخلفية الزرقاء والأبيض للنص ومسافات
        fontWeight: 700, fontSize: 14, // عرض الخط غامق
        display: 'flex', justifyContent: 'space-between', // وضع النص في جهة والأيقونة في الجهة المقابلة
        ...
        <div style={{ marginBottom: 12 }}>
          ...
          <div className="mobile-filter-row" style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 16, fontSize: 13 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', flex: 1, maxWidth: 400 }}>
              ...
              <input type="text" value={filter} onChange={e => { setFilter(e.target.value); setCurrentPage(1); }}
                className={styles.filterInputStyle} style={{ borderRight: 'none', borderTopRightRadius: 0, borderBottomRightRadius: 0, flex: 1, height: 28 }} />
            </div>
          </div>
          ...
          <div className="mobile-filter-row" style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 16, fontSize: 13, flexWrap: 'wrap' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              ...
              <select value={sortBy} onChange={e => { setSortBy(e.target.value); setCurrentPage(1); }} className={styles.filterSelectStyle}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              ...
              <select value={showAtLeast} onChange={e => { setShowAtLeast(Number(e.target.value)); setCurrentPage(1); }} className={styles.filterSelectStyle}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              ...
              <select value={showFilter} onChange={e => { setShowFilter(e.target.value); setCurrentPage(1); }} className={styles.filterSelectStyle}>
          ...
          <div className="mobile-bulk-row" style={{ display: 'flex', alignItems: 'flex-start', gap: 16, marginBottom: 8, fontSize: 13 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', flex: 1, maxWidth: 300 }}>
              ...
              <select value={bulkAction} onChange={e => { setBulkAction(e.target.value); setSelectedResource(null) }} className={styles.filterSelectStyle} style={{ minWidth: 200 }}>
                <option value=""></option>
